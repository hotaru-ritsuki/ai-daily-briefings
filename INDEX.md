...
- **[2026-06-19](briefings/2026-06-19.md)**
  - Claude Code v2.1.183 (Jun 19) adds auto-mode guardrails against destructive git (`reset --hard`, `checkout -- .`, `clean -fd`, `stash drop`), `commit --amend` on non-agent commits, and `terraform/pulumi/cdk destroy` without an explicit stack — plus a real auto-mode security fix where scheduled-task/webhook deliveries were treated as keyboard input and could approve a pending action or set the session title; v2.1.181 (Jun 17) adds `/config key=value`, caps foreground subagents to the 5-level depth limit, and fixes prompt caching on custom `ANTHROPIC_BASE_URL`/Foundry plus `awsCredentialExport` refresh churn
  - Codex `rust-v0.141.0` stable (Jun 18) ends the long alpha-empty-body streak with encrypted Noise relay channels for remote executors, per-thread plugin stdio-MCP activation, TLS P-521 for enterprise proxies, and hook-trust-bypass persistence through `codex exec` with PostToolUse correctly rejecting code-mode calls — the regulated-perimeter transport hardening; 0.142.0-alpha cycle restarted empty
  - GLM-5.2 open weights actually landed on Hugging Face under MIT (Jun 16) — 753B/40B-active MoE, 1M context, IndexShare sparse attention, single-source benchmarks SWE-bench Pro 62.1 (vs GPT-5.5 58.6) and within ~0.7pt of Opus 4.8 on FrontierSWE; meanwhile Fable 5/Mythos 5 remain down on day six with Anthropic "very confident" of restoration "in coming days" and a Commerce deal reportedly close (Kalshi ~57% by Jul 1), and Gemini 3.5 Pro still hasn't shipped
