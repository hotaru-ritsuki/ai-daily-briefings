...
- **[2026-06-15](briefings/2026-06-15.md)**
  - Claude Fable 5 disabled worldwide June 12 by US government export-control directive citing a verbal "narrow non-universal jailbreak" — three-day Mythos-class flagship life (June 9 → 12), Polymarket favours restoration by early July, all `claude-fable-5` pins now silently fall back to Opus 4.8 and the v2.1.170 changelog functionally describes an unreachable model
  - Claude Code v2.1.170–176 (seven releases in three days) shipped recursive sub-agents up to 5 levels deep (172), `enforceAvailableModels` managed setting that hard-blocks user/project widening of allowlists (175), per-skill/agent/plugin/MCP usage attribution in VSCode `/usage` (174), and a stack of `availableModels` policy-bypass fixes covering subagent overrides + agent-dispatch picker + advisor model + `/fast` toggle + `ANTHROPIC_DEFAULT_*_MODEL` env-var redirect; Codex `rust-v0.139.0` stable (June 9) gives Code mode standalone web search + preserves `oneOf`/`allOf` schemas through compaction, with 19 empty-bodied 0.140 alphas already queued
  - Vercel AI SDK 7 `HarnessAgent` (June 12) introduces a single API across Claude Code / Codex / Pi — normalising skills, sandboxes, sessions, sub-agents, permissions, and compaction; first credible "write once, switch harness" abstraction at the harness-above-the-model-call layer
