...
- **[2026-06-18](briefings/2026-06-18.md)**
  - Claude Code v2.1.181 (Jun 17) fixes prompt caching silently not reading on custom `ANTHROPIC_BASE_URL` / Foundry — a per-request attestation token was changing every turn, so gateway users may have been paying full input cost with zero cache hits; also caps foreground subagents at the 5-level depth limit, adds `/config key=value` from the prompt, `CLAUDE_CLIENT_PRESENCE_FILE`, Bun 1.4, and makes `claude mcp list` show "tools fetch failed" instead of false `✓ Connected`; v2.1.179 (Jun 16) preserves partial responses on mid-stream drops and fixes the WSL2 scroll regression
  ...
  - Anthropic shipped a Claude Design overhaul (Jun 17) with a `/design` command and Claude Code handoff (import a design system from GitHub, build against its components, self-correct); SpaceX Colossus compute deal + doubled Claude Code 5-hour limits reported but date unconfirmed; Gemini 3.5 Pro and GPT-5.6 still unshipped; MCP spec freeze dated Jul 28 (stateless core, first-class extensions, full JSON Schema for tools)
