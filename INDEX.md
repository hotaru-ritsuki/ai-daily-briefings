...
- **[2026-06-16](briefings/2026-06-16.md)**
  - Fable 5 and Mythos 5 suspended globally since 17:21 ET on June 12 under a US national-security export-control directive barring foreign-national access (including Anthropic's own foreign-national employees); unable to verify nationality in real time, Anthropic disabled both models for every customer worldwide — Opus 4.8 / Sonnet 4.6 / Haiku 4.5 unaffected; directly reverses 06-11's recommendation to pin `claude-fable-5` in `.claude/settings.json`
  - Claude Code v2.1.178 (June 15) ships the first parameter-scoped permission primitive — `Tool(param:value)` rules with `*` wildcards (`Agent(model:opus)`, `Bash(command:rm*)`), plus nested `.claude/` directory loading with `<dir>:<name>` collision resolution, Auto-mode classifier reviewing subagent spawns *pre-launch*, and v2.1.175's `enforceAvailableModels` managed setting making the allowlist a hard ceiling not just a UI hint
  - Codex `rust-v0.140.0` stable (June 15) ships `/import` from Claude Code (first vendor-built cross-tool migration utility), managed Amazon Bedrock API-key auth, and encrypted local storage for CLI and MCP OAuth credentials; the Agent SDK credit-pool split also went live June 15 — Pro $20 / Max 5× $100 / Max 20× $200, billed at API list rates, no rollover
